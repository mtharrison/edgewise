...
Like hover measurement: on pointer move or modifier change over a channel row, request `burstAt`; if one is in flight, remember only the newest. The request is made from each drawn frame, like #21's annotation hit test, so it follows pointer moves, modifier changes and view changes (after framing, the highlight shows the refined burst under the pointer). The result is kept as a separate `burst` field of the frame, next to #21's annotation highlight, and triggers a redraw only when it changes. It clears when the modifier is released, the window loses focus, or the pointer leaves the row.
